...
- 보안 대화상자 대응을 위해 서버H에서 보조 자동화 래퍼를 사용했다.
- 일부 파일은 COM 서버가 중간에 죽으며 `RPC 서버를 사용할 수 없습니다` 오류가 발생했다.
- 대표 실패 파일을 재검증한 결과, `SaveAs` 반환 후 실제 `.hwpx` 산출물이 남지 않는 사례가 있어 스크립트에 출력 파일 존재 검증을 추가했다.
- 최종 제외 `52`건은 수동 확인 결과 한/글에서 `배포용 문서`로 표시되어 다른 형식으로 저장할 수 없었다.
